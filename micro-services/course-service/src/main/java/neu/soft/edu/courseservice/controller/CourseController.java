package neu.soft.edu.courseservice.controller;

import cn.dev33.satoken.stp.StpUtil;
import io.swagger.annotations.Api;
import io.swagger.annotations.ApiOperation;
import neu.soft.edu.courseservice.pojo.*;
import neu.soft.edu.courseservice.service.*;
import neu.soft.edu.courseservice.util.tools.PropertiesUtil;
import neu.soft.edu.courseservice.util.tools.ResultVO;
...
public class CourseController {
    ...
    // ---------------------------------
    @GetMapping("/querySpecial/{courseId}")
    @ApiOperation("获取特殊课程")
    public Course querySpecialCourseById(@PathVariable int courseId) {
        return courseService.querySpecialCourseById(courseId);
    }

    @GetMapping("/queryOnlyCoach/{userId}/{courseId}")
    public CoachApply queryOnlyCoachApply(@PathVariable("userId") int userId, @PathVariable("courseId") int courseId) {
        return coachApplyService.queryOnlyCoachApply(userId, courseId);
    }

    @PostMapping("/add/CoachApply")
    public Boolean addCoachApply(@RequestBody CoachApply coachApply) {
        return coachApplyService.addCoachApply(coachApply);
    }
}
