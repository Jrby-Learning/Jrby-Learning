package neu.soft.edu.messageservice.controller;
...
import neu.soft.edu.messageservice.util.tools.ResultVO;
...
public class MessageController {
    ...
    // ------------------ 被其他微服务调用的接口 ---------------------
    @GetMapping("/system/newMessageState")
    @ApiOperation("查询是否有新消息")
    public Boolean newMessageState(int userId) {
        return myMessageService.newMessageState(userId);
    }

    @PostMapping("/system/insertMyMessage")
    @ApiOperation("查询是否有新消息")
    public Boolean insertMyMessage(@RequestBody MyMessage myMessage) {
        return myMessageService.insertMyMessage(myMessage);
    }
}
