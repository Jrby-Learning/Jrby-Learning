    ...
    <div class="resource-container">
      <el-card class="resource-card" shadow="never">
        ...
        <div class="resource-info">
          <el-alert title="下载资源需要扣除对应积分，下载后记得保存好文件，不然下次下载会继续扣除积分哦！" type="warning"  close-text="知道了" show-icon></el-alert>
          <el-table :data="resourceData" class="resource-table" height="382px" stripe border style="width: 100%;" empty-text="暂无学习资料">
            ...
            <el-table-column label="所需积分" width="180">
    ...
    methods:{
      ...
      download(row){
        if(this.$cookie.getToken()===null){
          ...
          this.$confirm('下载这份学习资料将扣除您'+row.coin+'积分, 是否继续?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            this.$courseApi.checkResourceExist(row.resourceId).then(res=>{
              this.$message.success("积分已扣除，请及时保存资料");
