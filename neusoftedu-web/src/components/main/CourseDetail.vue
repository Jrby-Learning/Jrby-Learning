    ...
    <div class="course-container">
      <div class="course-player">
        <div class="player-container" v-if="course != null">
          <div class="locked" v-if="locked">
            <h2>
              <el-image
                style="width: 100px;height: 100px;position: absolute;left: 240px"
                :src="logoImg"
              ></el-image>
              <span style="position: absolute;top: 50px;left: 350px;"
                >开通VIP，即可解锁全部章节</span
              >
            </h2>
            <el-button type="primary" round @click="openVip"
              >VIP免费看</el-button
            >
          </div>
          <video-player
          ...
          <div class="jump-play" v-if="playRecord != null">
            <el-button
              type="text"
              icon="el-icon-close"
              style="font-size: 20px;margin:-2px 0 0 12px;vertical-align: middle"
              @click="closeJump"
            ></el-button>
            <span>上次看到 {{ playRecord.listName }}</span>
            <el-button type="text" @click="autoPlay" style="margin: 0 14px;"
              >跳转播放</el-button
            >
          </div>
        </div>
        <div class="course-list">
          <div class="header">
            ...
            <p class="desc">
              <span>共{{ courseList.length }}节</span>
              <el-divider direction="vertical"></el-divider>

              <span
                >时长{{
                  course.courseTime | changeHourMin(course.courseSecond)
                }}</span
              >
          ...
          <div
            class="course-operate"
            v-if="
              this.$store.state.userInfo && this.$store.state.userInfo.userId
            "
          >
            <el-button type="primary" round @click="joinCourse"
              >加入课程</el-button
            >
          </div>
        </div>
      ...
      <div class="course-box">
        ...
        <div class="recommend">
          ...
          <ul class="content">
            <li
              v-for="(course, index) in recommendCourse"
              :key="index"
              @click="watch(course)"
            >
...
export default {
        ...
        sources: [
          {
            type: "video/mp4",
            src:
              "http://localhost:9000/upload/course/HTML5+CSS3%E9%9B%B6%E5%9F%BA%E7%A1%80%E7%89%B9%E8%AE%AD%E7%8F%AD/16204635255731.1.mp4"
          }
        ],
        poster: "", //你的封面地址
  ...
  created() {
    ...
    if (
      this.$store.state.vipInfo != null &&
      this.$store.state.vipInfo.isVip != null
    ) {
